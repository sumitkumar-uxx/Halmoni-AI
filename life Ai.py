...
file_four  = pd.read_csv(r"C:\Users\sumit\OneDrive\Pictures\Documents\Morning_Routine_Productivity.csv")
frame_four =  pd.DataFrame(file_four)
label_four = LabelEncoder()
frame_four["Meditation"]=label_four.fit_transform(frame_four["Meditation"])
frame_four["Breakfast"]=label.fit_transform(frame_four["Breakfast"])
inputs_four = frame_four[["Wake-up Time (Hour)","Meditation","Breakfast","Screen Time Before Work (min)"]]
output_four = frame_four["Productivity Score"]
x_train , x_test , y_train , y_test = train_test_split(inputs_four,output_four,random_state=42,train_size=0.8)
model = XGBRegressor()
model.fit(x_train,y_train)
joblib.dump(model,"model_four.pkl")
print(frame_four)
